...
time_step = 10 #simulation time steps
state_data_interval = 5 #interval steps for recording state data into StateData.log
traj_interval = 1 #interval steps for recording trajectories into traj.dcd
checkpoint_interval = 5 #interval steps for saving checkpoint file
continue_simulation = 'no' #yes: continue the simulation from previous results; no: begin a new simulation based on CONFIG.gro
continue_conf = 'config_eq_final.xml' #the input file you continue the simulation
...
system.addForce(AndersenThermostat(300*kelvin, 1/picosecond)) #Andersen thermostat
#system.addForce(MonteCarloBarostat(1*bar, 300*kelvin)) #Monte Carlo barostat
...
simulation.reporters.append(StateDataReporter('StateData.log', state_data_interval, step=True, volume=True,
    kineticEnergy=True, potentialEnergy=True, totalEnergy=True, temperature=True, separator=' '))

### Record trajectories ###
simulation.reporters.append(DCDReporter('traj.dcd', traj_interval)) 
...
PDBFile.writeFile(simulation.topology, final_positions, open('config_final.pdb', 'w'))
simulation.saveState('config_final.xml')
