...
def site_energy_calculation(system, simulation, ff, list_indx, list_QM_weight):
    ...
    for i, charge_idx in np.ndenumerate(list_indx):
      ##### Remove Force Field #####
      #print('total energy: %d %f %f' %(charge_idx, list_QM_weight[i], energy.getState(getEnergy=True).getPotentialEnergy()._value))
    ...
    site_energy = site_energy * 0.01036410 #change unit from kJ/mol to eV)
